...
class ComplexPresolver(PreSolver):
    ...
    def calculate_initial_conditions(self, circuit, user_initial_values):
        ...
        self._fill_nodes_with(self._p, cds, stop_cmps, self._FORWARD,
                                          self._calculated_p_cd_forward, n_iteration=1)
        self._fill_nodes_with(self._p, cds, stop_cmps, self._BACKWARD,
                                          self._calculated_p_cd_backward, n_iteration=1)
        self._fill_nodes_with(self._p, evs, stop_cmps, self._FORWARD,
                                          self._calculated_p_ev_forward, n_iteration=1)
        self._fill_nodes_with(self._p, evs, stop_cmps, self._BACKWARD,
                                          self._calculated_p_ev_backward, n_iteration=1)
        self._fill_nodes_with(self._p, xvs, stop_cmps, self._FORWARD,
        ...
        self._fill_nodes_with(self._h, cds, stop_cmps, self._FORWARD,
                                          self._calculated_h_cd_forward, n_iteration=1)
        self._fill_nodes_with(self._h, cds, stop_cmps, self._BACKWARD,
                                          self._calculated_h_cd_backward, n_iteration=1)
        self._fill_nodes_with(self._h, evs, stop_cmps, self._FORWARD,
                                          self._calculated_h_ev_forward, n_iteration=1)
        self._fill_nodes_with(self._h, cps, stop_cmps, self._FORWARD,
                                          self._calculated_h_cp_forward, n_iteration=1)
        self._fill_nodes_with(self._h, cps, stop_cmps, self._BACKWARD,
                                          self._calculated_h_cp_backward, n_iteration=1)
        self._fill_nodes_with(self._h, mfs, stop_cmps, self._FORWARD,
                                          self._calculated_h_mf_forward, n_iteration=1)
        self._fill_nodes_with(self._h, mfs, stop_cmps, self._BACKWARD,
        ...
        #self._circuit.update_mass_flows()
    ...
    def _fill_nodes_with(self, physic_property, start_cmps, stop_cmps, direction, calc_value,
                         n_iteration=1):
        ...
        while n_iteration > 0:
            ...
            for cmp_id in start_cmps:
                ...
                for n_id in n_to_fill:
                    ...
                    self._fill_next_nodes(n_id, cmp_explored, stop_cmps, n_not_filled, n_to_fill,
    ...
    def _fill_next_nodes(self, n_id, cmp_explored, stop_cmps, n_not_filled, n_to_fill,
        ...
        if not self._is_node_fill(n_id, physic_property):
            ...
            if c not in cmp_explored and c not in stop_cmps:
                ...
                n_to_fill += nodes_id
                self._fill_next_nodes(node_id, cmp_explored, stop_cmps, n_not_filled, n_to_fill,
                                      physic_property, value, direction)

    def _is_node_fill(self, node_id, physic_property):
    ...
    def _calculated_p_cd_forward(self, condenser, tc=318.15):
    ...
    def _calculated_p_cd_backward(self, condenser, tc=318.15):
    ...
    def _calculated_p_ev_forward(self, evaporator, te=263.15):
    ...
    def _calculated_p_ev_backward(self, evaporator, te=263.15):
    ...
    def _calculated_p_xv_forward(self, expansion_valve, te=273.15):
        ...
        if nd_id_1 is not None and nd_id_2 is not None:
            i = self._nd_values[nd_id_1].index(self._p)
            p1 = self._nd_values[nd_id_1][i + 1]
            i = self._nd_values[nd_id_2].index(self._p)
            p2 = self._nd_values[nd_id_2][i + 1]
            return (p1 * p2) ** 0.5
        else:
            return None

    def _search_compressor(self, n_id, cmp_explored, compressors, n_to_explore):
    ...
    def _calculated_h_cd_forward(self, condenser, tc=318.15):
    ...
    def _calculated_h_cd_backward(self, condenser, tc=318.15):
        # Calculated enthalpy for the condenser and nodes after it.
        # Use a initial value passed or one by default, condensation temperature = 45ºC.
        refrigerant = self._circuit.get_refrigerant()
        tmin = refrigerant.Tmin()
    ...
    def _calculated_h_ev_forward(self, evaporator, te=263.15):
    ...
    def _calculated_h_cp_forward(self, compressor):
        ...
        try:
            i = self._nd_values[inlet_node_id].index(self._p)
        except ValueError:
            return None
        p_suction = self._nd_values[inlet_node_id][i + 1]
        try:
            i = self._nd_values[inlet_node_id].index(self._h)
        except ValueError:
            return None
        h_suction = self._nd_values[inlet_node_id][i + 1]
        try:
            i = self._nd_values[outlet_node_id].index(self._p)
        except ValueError:
            return None
        p_discharge = self._nd_values[outlet_node_id][i + 1]
        refrigerant = self._circuit.get_refrigerant()
    ...
    def _calculated_h_cp_backward(self, compressor):
        ...
        p_discharge = self._nd_values[outlet_node_id][i + 1]
        try:
            i = self._nd_values[outlet_node_id].index(self._h)
        except ValueError:
            return None
        h_discharge = self._nd_values[outlet_node_id][i + 1]
        refrigerant = self._circuit.get_refrigerant()
        # Approximated.
    ...
    def _calculated_h_mf_forward(self, mixer_flow):
        ...
        for node in inlet_nodes:
            ...
            node_id = node.get_id()
            try:
                h_in += self._nd_values[node_id].index(self._h)
                i += 1
    ...
    def _calculated_h_mf_backward(self, mixer_flow):
        ...
        for node in outlet_nodes:
            ...
            node_id = node.get_id()
            try:
                h_out += self._nd_values[node_id].index(self._h)
                i += 1
            # No value in this node
            except ValueError:
                pass
    ...
    def _calculate_mass_flow(self):
        ...
        # TODO change when default argument are supported.
        for cd in cds:
            cd = self._circuit.get_component(cd)
            ...
            if 'heating_power' in properties:
                ...
                if self._mass_flows[id_mass_flow] is None:
                    self._mass_flows[id_mass_flow] = mass_flow
                id_mass_flow = outlet_node.get_id_mass_flow()
                if self._mass_flows[id_mass_flow] is None:
                    self._mass_flows[id_mass_flow] = mass_flow

        for ev in evs:
            ev = self._circuit.get_component(ev)
        ...
        for flow_component in flow_components:
            ...
            if None not in in_m and None in out_m:
                ...
                for outlet_node in outlet_nodes:
                    outlet_node = self._circuit.get_node(outlet_node)
                    id_mass_flow = outlet_node.get_id_mass_flow()
                    if self._mass_flows[id_mass_flow] is None:
                        self._mass_flows[id_mass_flow] = mass_flow
            ...
            if None not in out_m and None in in_m:
                ...
                for inlet_node in inlet_nodes:
                    inlet_node = self._circuit.get_node(inlet_node)
                    id_mass_flow = inlet_node.get_id_mass_flow()
                    if self._mass_flows[id_mass_flow] is None:
                        self._mass_flows[id_mass_flow] = mass_flow
